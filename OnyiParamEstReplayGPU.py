...
class HarData:
    ...
    def __init__(self,seq_len=16):
        ...
        train_ins = np.loadtxt(f"data/har/UCI HAR Dataset/trainSim/P{numID}_bolus.txt")
        train_meal = np.loadtxt(f"data/har/UCI HAR Dataset/trainSim/P{numID}_meal.txt")
        train_basal = np.loadtxt(f"data/har/UCI HAR Dataset/trainSim/P{numID}_basal.txt")
        ...
        test_ins = np.loadtxt(f"data/har/UCI HAR Dataset/trainSim/P{numID}_bolus.txt")
        test_meal = np.loadtxt(f"data/har/UCI HAR Dataset/trainSim/P{numID}_meal.txt")
        test_meal2 = test_meal
        test_basal = np.loadtxt(f"data/har/UCI HAR Dataset/trainSim/P{numID}_basal.txt")
...
class HarModel:
    ...
    def fit(self,gesture_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            ...
            if earlyStop :
                if best_loss > newLoss :
                    ...
                    if count > 10:
                        break
                if np.abs(best_loss - newLoss) < 0.02*best_loss and best_loss < 10:
                    count2 = count2 + 1
                elif np.abs(best_loss - newLoss) < 0.02*best_loss and best_loss >= 10 :
                    count3 = count3 + 1
                else:
                    count2 = 0
                if count2 > 5 :
        ...
        file_path = 'outputV2.csv'

        # Define the column labels and values
        columns = ['numID', 'test_loss', 'kempt', 'kabs','f', 'Gb', 'SG','Vg', 'p2', 'SI', 'Ipb', 'alpha', 'kd', 'beta', 'Vi', 'ka2', 'ke', 'bolusD', 'r2']
        values = [numID, test_loss, kempt, kabs, f, Gb, SG, Vg, p2, SI, Ipb, alpha, kd, beta, Vi, ka2, ke, bolusD, r2]  # Replace with your float values
        if test_loss < 40:
