...
class Custom_CE_Loss(tf.keras.losses.Loss):
    ...
    def lossFV2(self):
        ...
        for i in range(1, limitLoop, 1):
            ...
            max_inc = 200000.0  # max mg/dL per timestep you consider safe
            mask    = tf.logical_and(
            ...
            #old_G  = GVal[:, :, i-1]                    # shape [batch, â€¦]
            #raw_dG = old_G + tau * raw_mix              # same shape

            # 2) build a boolean mask of â€œsafeâ€ updates per example
            #    only those with |Î”G| <= 300 pass through
            mask   = tf.abs(diffDummy) <= 3000.0    # shape [batch, â€¦]

            # 3) pick either the new value or keep the old one, per element
            dummyG1  = tf.where(mask,
                              dummyG1,    # if within Â±300
    ...
    def fit(self,gesture_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            ...
            for batch_x,batch_y,batch_ins,batch_mL,batch_basal,batch_initIsc1,batch_initIsc2,batch_initIp,batch_BW,batch_u2ss in gesture_data.iterate_train(batch_size=16):
                ...
                lossPlot, GPlot, TPlot = Custom_CE_Loss(labels = batch_y.astype(np.float32),logits = t_y.astype(np.float32),insulin = batch_ins.astype(np.float32),meal=batch_mL.astype(np.float32),basal=batch_basal.astype(np.float32),initIsc1=batch_initIsc1.astype(np.float32),initIsc2=batch_initIsc2.astype(np.float32),initIp=batch_initIp.astype(np.float32),BW=batch_BW.astype(np.float32),u2ss=batch_u2ss.astype(np.float32)).lossFPlot()
                pdb.set_trace()
