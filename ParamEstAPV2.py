...
class Custom_CE_Loss(tf.keras.losses.Loss):
    ...
    def lossFV2(self):
        ...
        Isc1Val = self.y_initIsc1[:, :, 0]

        Isc2Val = self.y_initIsc2[:, :, 0]

        IpVal = self.y_initIp[:, :, 0]
        ...
        for i in range(1, limitLoop, 1):
            ...
            risk = tf.math.multiply(10 * tf.math.square(
                tf.math.pow(tf.math.log(GVal[:, :, i - 1]) , r2) - np.power(np.log(119.13), r2)),
        ...
        # print(xValD2)
        err = tf.math.sqrt(
            tf.math.reduce_sum(tf.math.square(self.y_true2[:, :, 0:limitLoop] - GVal) / limitLoop, axis=2))
    ...
    def lossF(self):        
        ...
        for i in range(1,limitLoop,1):
            ...
            dummyIsc1 = Isc1Val[:,:,i-1] + tau*(tf.math.add(-tf.math.multiply(kd,Isc1Val[:,:,i-1]),(tf.math.add(self.y_basal[:,:,i-1],self.y_ins[:,:,i-1]))/Vi))
            #kd_reshaped = tf.broadcast_to(kd, tf.shape(Isc1Val[:, :, 0]))
            #dummyIsc1 = Isc1Val[:, :, i - 1] + tau * (-tf.math.multiply(kd_reshaped, Isc1Val[:, :, i - 1]))
            dummyIsc2 = Isc2Val[:,:,i-1] + tau*(tf.math.multiply(kd,Isc1Val[:,:,i-1])-tf.math.multiply(ka2,Isc2Val[:,:,i-1]))
            dummyIp = IpVal[:,:,i-1] + tau*(tf.math.multiply(ka2,Isc2Val[:,:,i-1])-ke*IpVal[:,:,i-1])
            dummyQsto1 = Qsto1Val[:,:,i-1] + tau*(-tf.math.multiply(kempt,Qsto1Val[:,:,i-1])+self.y_meal[:,:,i-1])
            dummyQsto2 = Qsto2Val[:,:,i-1] + tau*(tf.math.multiply(kempt,Qsto1Val[:,:,i-1])-tf.math.multiply(kempt,Qsto2Val[:,:,i-1]))
            dummyQgut = QgutVal[:,:,i-1] + tau*(tf.math.multiply(kempt,Qsto2Val[:,:,i-1])-tf.math.multiply(kabs,QgutVal[:,:,i-1]))
            RatVal = f*tf.math.multiply(kabs,QgutVal[:,:,i-1])
            #print("risk is {}".format(risk))
            dummyXVal = xVal[:,:,i-1] + tau*(-tf.math.multiply(p2,xVal[:,:,i-1])-tf.math.multiply(SI,(IpVal[:,:,i-1]-Ipb)))
            dummygVal = gVal[:,:,i-1] + tau*(-tf.math.multiply(SG+tf.math.multiply(risk,xVal[:,:,i-1]),gVal[:,:,i-1])+tf.math.multiply(SG,Gb)+(RatVal/Vg))
...
class HarData:
    ...
    def __init__(self,seq_len=16):
        ...
        train_x = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_glucoseV2.txt")
        train_y = (np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_glucoseV2.txt")-1)#.astype(np.int32)
        train_ins = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_bolus.txt")
        train_meal = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_meal.txt")
        train_basal = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_basal.txt")
        train_initIsc1 = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_Isc1.txt")
        train_initIsc2 = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_Isc2.txt")
        train_initIp = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_Ip.txt")
        train_BW = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_BW.txt")
        train_u2ss = np.loadtxt(f"data/har/UCI HAR Dataset/train/Virtual_570_{numID}_u2ss.txt")
        ...
        test_x = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_glucoseV2.txt")
        test_y = (np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_glucoseV2.txt")-1)#.astype(np.int32)
        test_ins = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_bolus.txt")
        test_meal = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_meal.txt")
        test_meal2 = test_meal
        test_basal = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_basal.txt")
        test_initIsc1 = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_Isc1.txt")
        test_initIsc2 = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_Isc2.txt")
        test_initIp = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_Ip.txt")
        test_BW = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_BW.txt")
        test_u2ss = np.loadtxt(f"data/har/UCI HAR Dataset/test/Virtual_570_{numID}_u2ss.txt")
...
class HarModel:
    ...
    def __init__(self,model_type,model_size,learning_rate = 0.001):
        ...
        self.lossV2 = Custom_CE_Loss(labels=self.target_y, logits=self.y, insulin=self.ins, meal=self.mL, basal=self.basal,
                           initIsc1=self.initIsc1, initIsc2=self.initIsc2, initIp=self.initIp, BW=self.BW,
                           u2ss=self.u2ss).lossFV2()
    ...
    def fit(self,gesture_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            ...
            if earlyStop :
                if best_loss > newLoss :
                    ...
                    if count > 10:
                        break
                if np.abs(best_loss - newLoss) < 0.05*best_loss and best_loss < 15:
                    count2 = count2 + 1
                elif np.abs(best_loss - newLoss) < 0.05*best_loss and best_loss >= 15 :
                    count3 = count3 + 1
                else:
                    count2 = 0
                if count2 > 5 :
        ...
        columns = ['numID' 'test_loss', 'kempt', 'kabs', 'Gb', 'SG', 'p2', 'SI', 'kd', 'ka2']
        values = [numID, test_loss, kempt, kabs, Gb, SG, p2, SI, kd, ka2]  # Replace with your float values
        if test_loss < 40:
...
if __name__ == "__main__":
    ...
    test_x = np.loadtxt(f"data/har/UCI HAR Dataset/test/Train_570_{numID}_glucose.txt")
    Nloop = test_x.shape[1]
