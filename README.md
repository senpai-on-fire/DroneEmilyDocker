...
Please run the following two lines to run the dockerized implementation of Emily and get back coefficients.csv that has the aerodynamic variables of a drone from simulation data

sudo docker build -t emily-drone:tf1 .
sudo docker run --rm -it -v "$PWD":/app emily-drone:tf1
